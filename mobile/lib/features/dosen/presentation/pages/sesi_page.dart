...
class _DosenSesiPageState extends State<DosenSesiPage> {
  ...
  Future<void> _initSessionFlow() async {
    ...
    try {
      ...
      if (activeSesi != null) {
        ...
        // 🔹 Tidak Ada Sesi Aktif, Mari Dapatkan Koordinat & Buat Baru!
  ...
  Future<void> _updateStatusManual(int absensiId, String targetStatus) async {
    ...
    try {
      final ok = await _dosenService.updateStatusManual(absensiId, targetStatus);
      if (ok) {
  ...
  Widget build(BuildContext context) {
    ...
    final totalHadir = _attendances.where((a) => a['status'] == 'Hadir' || a['status'] == 'Terlambat').length;
